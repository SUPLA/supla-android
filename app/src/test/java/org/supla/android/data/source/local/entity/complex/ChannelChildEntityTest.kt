...
class ChannelChildEntityTest {
  ...
  fun `check if the indicator icon of children is calculated properly - standby and heating`() {
    // given
    val firstStandby = mockChild(ThermostatIndicatorIcon.STANDBY)
    val secondStandby = mockChild(ThermostatIndicatorIcon.STANDBY)
    val thirdHeating = mockChild(ThermostatIndicatorIcon.HEATING)
  ...
  fun `check if the indicator icon of children is calculated properly - standby, off by sensor, heating`() {
    // given
    val firstStandby = mockChild(ThermostatIndicatorIcon.STANDBY)
    val secondOffBySensor = mockChild(ThermostatIndicatorIcon.FORCED_OFF_BY_SENSOR)
    val thirdHeating = mockChild(ThermostatIndicatorIcon.HEATING)
  ...
  fun `check if the indicator icon of children is calculated properly - standby, off by sensor, off`() {
    ...
    val firstStandby = mockChild(ThermostatIndicatorIcon.STANDBY)
    val secondOffBySensor = mockChild(ThermostatIndicatorIcon.FORCED_OFF_BY_SENSOR)
    val thirdOff = mockChild(ThermostatIndicatorIcon.OFF)
  ...
  private fun mockChild(indicatorIcon: ThermostatIndicatorIcon): ChannelChildEntity =
    mockk {
