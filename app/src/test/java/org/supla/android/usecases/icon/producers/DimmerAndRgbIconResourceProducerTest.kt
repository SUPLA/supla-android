...
class DimmerAndRgbIconResourceProducerTest : BaseIconResourceProducerTest() {
  ...
  @Test
  fun `should produce off off icon (night mode)`() {
    test(
      data = mockIconData(
        state = ChannelState(ChannelState.Value.COMPLEX, listOf(ChannelState.Value.OFF, ChannelState.Value.OFF)),
        nightMode = true
      ),
      function = SuplaConst.SUPLA_CHANNELFNC_DIMMERANDRGBLIGHTING,
      expectedIcon = R.drawable.dimmerrgboffoff_nightmode
    )
  }

  @Test
  fun `should produce on off icon (night mode)`() {
    test(
      data = mockIconData(
        state = ChannelState(ChannelState.Value.COMPLEX, listOf(ChannelState.Value.ON, ChannelState.Value.OFF)),
        nightMode = true
      ),
      function = SuplaConst.SUPLA_CHANNELFNC_DIMMERANDRGBLIGHTING,
      expectedIcon = R.drawable.dimmerrgbonoff_nightmode
    )
  }

  @Test
  fun `should produce off on icon (night mode)`() {
    test(
      data = mockIconData(
        state = ChannelState(ChannelState.Value.COMPLEX, listOf(ChannelState.Value.OFF, ChannelState.Value.ON)),
        nightMode = true
      ),
      function = SuplaConst.SUPLA_CHANNELFNC_DIMMERANDRGBLIGHTING,
      expectedIcon = R.drawable.dimmerrgboffon_nightmode
    )
  }

  @Test
  fun `should produce on on icon (night mode)`() {
    test(
      data = mockIconData(
        state = ChannelState(ChannelState.Value.COMPLEX, listOf(ChannelState.Value.ON, ChannelState.Value.ON)),
        nightMode = true
      ),
      function = SuplaConst.SUPLA_CHANNELFNC_DIMMERANDRGBLIGHTING,
      expectedIcon = R.drawable.dimmerrgbonon_nightmode
    )
  }
}
