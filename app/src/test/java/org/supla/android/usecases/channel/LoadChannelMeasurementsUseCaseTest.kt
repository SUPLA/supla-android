...
class LoadChannelMeasurementsUseCaseTest : BaseLoadMeasurementsUseCaseTest() {
  ...
  fun `should provide sets when there is a provider which can handle given function`() {
    ...
    val channelSets: ChannelSets = mockk()
    ...
    every { temperatureMeasurementsProvider.provide(channel, spec) } returns Single.just(channelSets)
    ...
    observer.assertResult(channelSets)
