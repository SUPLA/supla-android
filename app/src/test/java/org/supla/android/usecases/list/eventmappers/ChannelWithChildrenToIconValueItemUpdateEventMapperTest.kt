...
class ChannelWithChildrenToIconValueItemUpdateEventMapperTest {
  ...
  fun `should map channel data to item`() {
    ...
    every { getChannelIssuesForListUseCase.invoke(shareableChannelWithChildren) } returns ListItemIssues.empty
    every { downloadEventsManager.getLastChannelDownloadState(1) } returns DownloadEventsManager.State.InProgress(10f)
    ...
    assertThat(defaultItem.processing).isTrue
  }
}
