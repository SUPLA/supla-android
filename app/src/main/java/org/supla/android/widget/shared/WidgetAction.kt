...
fun WidgetAction.getActionId(configuration: WidgetConfiguration): ActionId? =
  when (this) {
    MANUAL_UPDATE, REDRAW, AUTOMATIC_UPDATE -> null
    BUTTON_PRESSED -> configuration.actionId ?: getDefaultAction(configuration.subjectFunction)
    LEFT_BUTTON_PRESSED -> getLeftButtonAction(configuration.subjectFunction)
    RIGHT_BUTTON_PRESSED -> getRightButtonAction(configuration.subjectFunction)
  }
...
private fun getDefaultAction(function: SuplaFunction): ActionId? =
  when (function) {
    SuplaFunction.OPEN_SENSOR_GATEWAY,
    SuplaFunction.OPEN_SENSOR_GATE,
    SuplaFunction.OPEN_SENSOR_GARAGE_DOOR,
