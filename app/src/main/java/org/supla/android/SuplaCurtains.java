...
public class SuplaCurtains extends View {
  ...
  private final DisplayMetrics Metrics =
      getResources() == null ? null : getResources().getDisplayMetrics();
  ...
  private void drawCurtain(
      Canvas canvas, float left, float right, float bottomPercent, Paint colorPaint) {
    MainRoundRect.set(
        workSpace.left + left,
  ...
  private void drawCurtains(Canvas canvas, boolean right) {
    ...
    if (right) {
      drawCurtain(
          canvas,
          workSpace.width() - mCurtainMargin - mElWidth,
          workSpace.width() - mCurtainMargin,
          (float) (1 - mLineSubtract),
          greenPaint);
    } else {
      drawCurtain(
          canvas,
          mCurtainMargin,
          mCurtainMargin + mElWidth,
          (float) (1 - mLineSubtract),
          greenPaint);
    }
  }
    ...
    switch (event.getAction()) {
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (((workSpace.height() * 1) + ((getHeight() - workSpace.height()) / 2) > mFirstClick)
            && ((workSpace.height() * 0.08 + ((getHeight() - workSpace.height()) / 2)
                < mFirstClick))) {
  ...
  private void calculateConstants() {
    ...
    setCourtainWidth(
        (((workSpace.width() - mCurtainMargin * 2) / 2f) - mElWidth) * mPercent / 100f);
  }
  ...
  public void onDraw(Canvas canvas) {
    ...
    // Windowsill L
    drawRoundRect(canvas, 0.145f, 0.87f, 0.16f, 0.96f, 3, 3, paint);

    // Windowsill R
    drawRoundRect(canvas, 0.84f, 0.87f, 0.855f, 0.96f, 3, 3, paint);

    // Cornice T
    drawRoundRect(canvas, 0.15f, 0.11f, 0.85f, 0.125f, 8, 8, paint);

    // Cornice B
    drawRoundRect(canvas, 0.18f, 0.125f, 0.82f, 0.16f, 3, 3, paint);
    ...
    // Flowerpot stand
    drawRoundRect(canvas, 0.6f, 0.87f, 0.71f, 0.88f, 3, 3, paint);

    // Flowerpot frame
    drawRoundRect(canvas, 0.595f, 0.785f, 0.715f, 0.8f, 7, 7, paint);

    // Leaf L L
    MainPath.reset();
    MainPath.moveTo(
        workSpace.width() * 0.62f + workSpace.left, workSpace.height() * 0.785f + workSpace.top);
    MainPath.quadTo(
    ...
    // Leaf C L
    MainPath.moveTo(
        workSpace.width() * 0.635f + workSpace.left, workSpace.height() * 0.785f + workSpace.top);
    MainPath.quadTo(
    ...
    // Leaf C R
    MainPath.moveTo(
        workSpace.width() * 0.65f + workSpace.left, workSpace.height() * 0.73f + workSpace.top);
    MainPath.quadTo(
    ...
    MainPath.quadTo(
        ...
        workSpace.height() * 0.785f + workSpace.top);
    MainPath.lineTo(
        workSpace.width() * 0.66f + workSpace.left, workSpace.height() * 0.785f + workSpace.top);
    MainPath.close();
    canvas.drawPath(MainPath, greenPaint);
    canvas.drawPath(MainPath, paint);
