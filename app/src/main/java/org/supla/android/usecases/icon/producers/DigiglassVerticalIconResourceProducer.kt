...
class DigiglassVerticalIconResourceProducer : IconResourceProducer {
  override fun accepts(function: SuplaFunction): Boolean =
    when (function) {
      SuplaFunction.DIGIGLASS_VERTICAL -> true
  ...
  override fun produce(data: IconData): Int =
    when (data.altIcon) {
      1 -> if (data.state.value == ChannelState.Value.OPAQUE) {
        R.drawable.fnc_digiglass_vertical_1
      } else {
        R.drawable.fnc_digiglass_vertical_1_transparent
      }

      else -> if (data.state.value == ChannelState.Value.OPAQUE) {
        R.drawable.fnc_digiglass_vertical
      } else {
        R.drawable.fnc_digiglass_vertical_transparent
      }
    }
}
