...
class ElectricityMeterValueFormatter : ChannelValueFormatter {
  ...
  override fun format(value: Any, withUnit: Boolean, precision: Int): String {
    ...
    if (doubleValue <= ElectricityMeterValueProvider.UNKNOWN_VALUE) {
      return NO_VALUE_TEXT
    }

    return if (withUnit) {
      String.format("%.${precision}f kWh", doubleValue)
    } else {
