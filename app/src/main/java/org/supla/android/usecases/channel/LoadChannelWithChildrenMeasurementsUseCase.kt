...
class LoadChannelWithChildrenMeasurementsUseCase @Inject constructor(
  ...
  private fun buildDataSets(
    ...
    val observables = mutableListOf<Single<ChannelSets>>().also { list ->
    ...
    return Single.zip(observables) { items ->
      mutableListOf<ChannelSets>().also { list ->
        items.forEach {
          (it as? ChannelSets)?.let { channelSets -> list.add(channelSets) }
        }
      }
    }
  }
