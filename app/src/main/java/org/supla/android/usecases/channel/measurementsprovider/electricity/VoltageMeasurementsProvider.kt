...
class VoltageMeasurementsProvider @Inject constructor(
  ...
  private fun findMeasurementsForPhase(
    ...
    voltageLogRepository.findMeasurements(channel.remoteId, channel.profileId, spec.startDate, spec.endDate, phase)
      .map { aggregating(it, spec.aggregation) }
      .map { Pair(phase, historyDataSet(channel, phase, isFirst, ChartEntryType.VOLTAGE, spec.aggregation, it)) }
}
