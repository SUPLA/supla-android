...
class FacadeBlindIconResourceProducer : IconResourceProducer {
  override fun accepts(function: Int): Boolean =
    when (function) {
      SUPLA_CHANNELFNC_CONTROLLINGTHEFACADEBLIND -> true
  ...
  override fun produce(data: IconData): Int =
    when (data.state.value) {
      ChannelState.Value.OPEN -> R.drawable.fnc_facade_blind_open
      else -> R.drawable.fnc_facade_blind_close
    }
}
