...
data class PowerActiveChartCustomData(
...
class PowerActiveMeasurementsProvider @Inject constructor(
  ...
  private fun findMeasurementsForPhase(
    ...
    powerActiveLogRepository.findMeasurements(channel.remoteId, channel.profileId, spec.startDate, spec.endDate, phase)
      .map { aggregating(it, spec.aggregation) }
      .map { Pair(phase, historyDataSet(channel, phase, isFirst, ChartEntryType.POWER_ACTIVE, spec.aggregation, it)) }
}
