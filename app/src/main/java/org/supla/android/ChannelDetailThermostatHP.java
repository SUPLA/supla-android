...
public class ChannelDetailThermostatHP extends DetailLayout
  ...
  public ChannelDetailThermostatHP(
    ...
    cfgItems =
        Arrays.asList(
            new CfgItem(
                ...
                1),
            new CfgItem(
                CfgItem.ID_WATER_MAX,
                R.id.hpBtnWaterMaxMinus,
                R.id.hpBtnWaterMaxPlus,
                R.id.hpTvWaterMax,
                30,
                70,
                70),
            new CfgItem(
                CfgItem.ID_ECO_REDUCTION,
                R.id.hpBtnEcoReductionMinus,
                R.id.hpBtnEcoReductionPlus,
                R.id.hpTvEcoReduction,
                1,
                5,
                3),
            new CfgItem(
                CfgItem.ID_TEMP_COMFORT,
                R.id.hpBtnAutoMinus,
                R.id.hpBtnAutoPlus,
                R.id.hpTvAuto,
                10,
  ...
  private byte setBtnAppearance(Button btn, int setOn, int textOn, int textOff) {
    ...
    if (setOn == BTN_SET_ON) {
      ...
      btn.setBackgroundResource(
          setOn == BTN_SET_OFF_UNKNOWN ? R.drawable.hp_button_unknown : R.drawable.hp_button_off);
      if (textOff != 0) {
        btn.setText(textOff);
      }
    }
  ...
  private void OnChannelGroupDataChanged() {
    ChannelGroup channelGroup =
        channelGroupRepository.findGroupDataEntity(getRemoteId()).blockingFirst().getLegacyGroup();
  ...
  public void OnChannelDataChanged() {
    ...
    Channel channel =
        channelRepository.findChannelDataEntity(getRemoteId()).blockingFirst().getLegacyChannel();
    ...
    if (!mCalendar.isTouched()) {
      ...
      if (thermostat.getSchedule() != null
          && thermostat.getSchedule().getType()
              == SuplaChannelThermostatValue.Schedule.TYPE_PROGRAM) {
        for (short d = 0; d < 7; d++) {
          for (short h = 0; h < 24; h++) {
            mCalendar.setHourProgramTo1(
                (short) (d + 1),
                h,
                thermostat.getSchedule().getValue((byte) d, (byte) h) == PROG_COMFORT);
          }
        }
      }
    }
      ...
      if (item.onClick(view)) {
        ...
        switch (item.getId()) {
          ...
          case CfgItem.ID_TURBO_TIME:
            deviceCalCfgRequest(SuplaConst.SUPLA_THERMOSTAT_CMD_SET_TIME, (byte) item.getValue());
  ...
  @Override
  public void onHourValueChanged(
      ...
      if (cursor.moveToFirst()) {
        ...
        do {
          ...
          if (setBtnOnOff != BTN_SET_OFF_UNKNOWN
              && setBtnOnOff != (thermostat.isThermostatOn() ? BTN_SET_ON : BTN_SET_OFF)) {
            setBtnOnOff = BTN_SET_OFF_UNKNOWN;
          }

          if (setBtnNormal != BTN_SET_OFF_UNKNOWN
              && setBtnNormal != (thermostat.isNormalOn() ? BTN_SET_ON : BTN_SET_OFF)) {
            setBtnNormal = BTN_SET_OFF_UNKNOWN;
          }

          if (setBtnEco != BTN_SET_OFF_UNKNOWN
              && setBtnEco != (thermostat.isEcoRecuctionApplied() ? BTN_SET_ON : BTN_SET_OFF)) {
            setBtnEco = BTN_SET_OFF_UNKNOWN;
          }

          if (setBtnTurbo != BTN_SET_OFF_UNKNOWN
              && setBtnTurbo != (thermostat.isTurboOn() ? BTN_SET_ON : BTN_SET_OFF)) {
