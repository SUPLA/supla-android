...
public class SuplaColorBrightnessPicker extends View {
  ...
  private static final double m160d = Math.toRadians(-160);
  private static final double m90d = Math.toRadians(-90);
  ...
  private void drawWheelMarkers(
    ...
    for (int a = 0; a < markers.size(); a++) {
      ...
      drawMarker(
          canvas, (float) Math.cos(angle) * radius, (float) Math.sin(angle) * radius, markerSize);
    }
  }
  ...
  private void drawSliderMarkers(Canvas canvas, float markerSize) {
    ...
    for (int a = 0; a < brightnessMarkers.size(); a++) {
      drawMarker(canvas, 0, h / 2 - h * brightnessMarkers.get(a).floatValue() / 100f, markerSize);
    }
  }
  ...
  private void drawCirclePointer(
      Canvas canvas, double angle, float wheelRadius, int color, PointF center) {
  ...
  private void drawPowerButton(Canvas canvas, float wheelRadius) {
    ...
    RectF rect =
        new RectF(-powerButtonRadius, -powerButtonRadius, powerButtonRadius, powerButtonRadius);
  ...
  private void drawWheel(Canvas canvas) {
    if (colorWheelVisible) {
      ...
      if (circleInsteadArrow) {
        drawCirclePointer(
            canvas, colorWheelPointerAngle, colorWheelRadius, selectedColor, colorPointerCenter);
      } else {
        drawArrow(
            canvas,
            colorPointerCenter,
    ...
    if (circleInsteadArrow) {
      ...
      drawCirclePointer(
          ...
          brightnessWheelRadius,
          calculateColor(angle, BW),
          brightnessPointerCenter);
    } else {
      drawArrow(
          canvas,
          brightnessPointerCenter,
  ...
  private void drawSlider(Canvas canvas) {
    ...
    if (slShader == null) {
      slShader =
          new LinearGradient(
              0, height / -3, 0, height / 2, Color.WHITE, Color.BLACK, Shader.TileMode.CLAMP);
  ...
  private void drawArrow(
      ...
      float wheelWidth,
      double arrowOffset,
      double arrowHeight_a,
      double arrowHeight_b,
      int color,
      Path arrowPath,
      Paint arrowPaint) {
  ...
  private void setBWcolor() {
    int color =
        colorWheelVisible && colorfulBrightnessWheel && !sliderVisible
            ? selectedColor
            : Color.WHITE;
  ...
  private boolean touchOverPointer(PointF touchPoint, PointF pointerCenter, double pointerHeight) {
    return Math.sqrt(
            Math.pow(pointerCenter.x - touchPoint.x, 2)
                + Math.pow(pointerCenter.y - touchPoint.y, 2))
        <= pointerHeight / 2;
  }
  ...
  private boolean touchOverColorWheel(PointF touchPoint) {
    if (colorWheelVisible) {
      ...
      return !touchOverPointer(touchPoint, center, colorWheelRadius * 2 - colorWheelWidth)
          && touchOverPointer(touchPoint, center, colorWheelRadius * 2 + colorWheelWidth);
    }
  ...
  private boolean touchOverBrightnssWheel(PointF touchPoint) {
    PointF center = new PointF(0, 0);
    return !touchOverPointer(touchPoint, center, brightnessWheelRadius * 2 - brightnessWheelWidth)
        && touchOverPointer(touchPoint, center, brightnessWheelRadius * 2 + brightnessWheelWidth);
  }
  ...
  private void moveColorPointerToAngle(double angle) {
    ...
    if (newColor != selectedColor) {
      ...
      if (mOnChangeListener != null) mOnChangeListener.onColorChanged(this, selectedColor);
    }
  }
  ...
  private void moveBrightnessPointerToAngle(double angle) {
    ...
    if (mOnChangeListener != null) mOnChangeListener.onBrightnessChanged(this, selectedBrightness);
  }
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_UP:
        ...
        if (powerButtonTouched) {
          ...
          if (mOnChangeListener != null) mOnChangeListener.onPowerButtonClick(this);
      ...
      case MotionEvent.ACTION_DOWN:
        ...
        if (!sliderVisible
            && powerButtonVisible
            && powerButtonEnabled
            && touchOverPointer(touchPoint, new PointF(0, 0), powerButtonRadius * 2.2)) {
      ...
      case MotionEvent.ACTION_MOVE:
        if (sliderVisible) {
          ...
          if (brightnessWheelPointerAngle >= m160d && brightnessWheelPointerAngle <= m20d) {
            ...
            if (brightnessWheelPointerAngle > newAngle) {
              if (brightnessWheelPointerAngle >= m90d && newAngle < m90d) {
                newAngle = m90d;
              }
            } else if (brightnessWheelPointerAngle < newAngle) {
              if (brightnessWheelPointerAngle <= m90_01d && newAngle > m90_01d) {
                newAngle = m90_01d;
              }
            }
          }
  ...
  public void setColorMarkers(ArrayList<Double> colorMarkers) {
    this.colorMarkers = colorMarkers == null ? null : new ArrayList<>(colorMarkers);
  ...
  public void setBrightnessMarkers(ArrayList<Double> brightnessMarkers) {
    this.brightnessMarkers = brightnessMarkers == null ? null : new ArrayList<>(brightnessMarkers);
    invalidate();
  }
