...
class LineChartData(
  ...
  override fun newInstance(sets: List<ChannelSets>): ChartData = LineChartData(dateRange!!, chartRange!!, aggregation!!, sets)
    ...
    LineDataSet(set, "").apply {
      ...
      when (label) {
        ...
        is HistoryDataSet.Label.Multiple -> {
          colors = label.values.map { ResourcesCompat.getColor(resources, it.color, null) }
          circleColors = colors
        }
      }
