...
data class ChannelSets(
  ...
  fun toggleActive(type: ChartEntryType): ChannelSets =
    copy(
  ...
  fun deactivate(): ChannelSets = copy(dataSets = dataSets.map { it.copy(active = false) })

  fun setActive(types: List<ChartEntryType>?): ChannelSets =
    copy(dataSets = dataSets.map { it.copy(active = types?.contains(it.type) == true) })

  fun empty(): ChannelSets = copy(dataSets = dataSets.map { it.copy(entities = emptyList()) })
  ...
  companion object {
    ...
    ) = ChannelSets(channel.remoteId, channel.function.suplaFunction(), name, aggregation, dataSets)
  }
}
