...
class CurrentLogRepository @Inject constructor(
  ...
  fun findMeasurements(
    ...
    return currentLogDao.findMeasurements(remoteId, profileId, startDate.time, endDate.time, phase)
  }
  ...
  override fun findMinTimestamp(remoteId: Int, profileId: Long): Single<Long> =
    currentLogDao.findMinTimestamp(remoteId, profileId)

  override fun findMaxTimestamp(remoteId: Int, profileId: Long): Single<Long> =
    currentLogDao.findMaxTimestamp(remoteId, profileId)

  override fun findOldestEntity(remoteId: Int, profileId: Long): Maybe<CurrentHistoryLogEntity> =
    currentLogDao.findOldestEntity(remoteId, profileId)

  override fun delete(remoteId: Int, profileId: Long): Completable =
    currentLogDao.delete(remoteId, profileId)

  override fun findCount(remoteId: Int, profileId: Long): Maybe<Int> =
    currentLogDao.findCount(remoteId, profileId)

  override fun insert(entries: List<CurrentHistoryLogEntity>): Completable =
    currentLogDao.insert(entries)

  override fun map(entry: HistoryMeasurement, groupingString: String, remoteId: Int, profileId: Long) =
    CurrentHistoryLogEntity(
      id = 0,
      channelId = remoteId,
      date = entry.date,
  ...
  override fun hasEmptyGroupingString(remoteId: Int, profileId: Long): Single<Boolean> =
    currentLogDao.hasEmptyGroupingString(remoteId, profileId)
