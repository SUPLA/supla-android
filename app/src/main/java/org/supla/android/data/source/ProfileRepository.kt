...
@Singleton
class ProfileRepository @Inject constructor(private val profileDao: ProfileDao) : CountProvider {

  fun insert(profileEntity: ProfileEntity) = profileDao.insert(profileEntity)

  fun update(profileEntity: ProfileEntity) = profileDao.update(profileEntity)
  ...
  fun updatePreferredProtocolVersion(profileEntity: ProfileEntity, version: Int) =
    update(profileEntity.copy(preferredProtocolVersion = version))

  fun updateServerForEmail(profileEntity: ProfileEntity, serverAddress: String) =
    update(profileEntity.copy(serverForEmail = serverAddress))
}
