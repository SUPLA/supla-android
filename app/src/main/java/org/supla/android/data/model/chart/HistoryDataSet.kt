...
data class HistoryDataSet(
  ...
  fun asBarChartData(
    ...
    return mutableListOf<IBarDataSet>().apply {
      entities.forEach { aggregatedEntities ->
        val entries = aggregatedEntities.map { entity ->
          val x = timeToCoordinateConverter?.let { it(entity.date.toFloat()) } ?: entity.date.toFloat()
          toBarEntry(x, aggregation, entity)
        }
  ...
  private fun toBarEntry(x: Float, aggregation: ChartDataAggregation, entity: AggregatedEntity): BarEntry =
    when (val value = entity.value) {
      is AggregatedValue.Single -> BarEntry(x, value.value, chartEntryDetails(aggregation, entity))
      is AggregatedValue.Multiple -> BarEntry(x, value.values, chartEntryDetails(aggregation, entity))
    }
  ...
  private fun chartEntryDetails(aggregation: ChartDataAggregation, entity: AggregatedEntity) =
    when (val value = entity.value) {
      is AggregatedValue.Single ->
        ChartEntryDetails(aggregation, type, entity.date, value.min, value.max, value.open, value.close, valueFormatter)

      is AggregatedValue.Multiple ->
        ChartEntryDetails(aggregation, type, entity.date, null, null, null, null, valueFormatter)
    }
