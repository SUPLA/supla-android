...
public class SuplaRoofWindowController extends View {
  ...
  private void init() {
    ...
    if (r != null) {
      px = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1.5f, r.getDisplayMetrics());
    }
  ...
  public SuplaRoofWindowController(
  ...
  private void drawOuterFrameMainPart(
      Canvas canvas,
      float centerX,
      float centerY,
      float frameWidth,
    ...
    float[] points =
        new float[] {
          ...
          frameWidth / -2,
          frameHeight / 2,
          frameWidth / -2 + bottomFramePostWidth,
          frameHeight / 2 - bottomFrameBarWidth,
          frameWidth / 2 - bottomFramePostWidth,
          frameHeight / 2 - bottomFrameBarWidth,
          frameWidth / 2 - bottomFramePostWidth,
          0,
          frameWidth / 2 - framePostWidth,
          0,
          frameWidth / 2 - framePostWidth,
          frameHeight / -2 + frameBarWidth,
          frameWidth / -2 + framePostWidth,
          frameHeight / -2 + frameBarWidth,
          frameWidth / -2 + framePostWidth,
          0,
    ...
    for (int a = 0; a < points.length - 1; a += 2) {
      if (a == 0 || a == 10 || (excludeInnerLines && ((a >= 12 && a <= 16) || a == 24))) {
        path.moveTo(points[a], points[a + 1]);
      } else {
        path.lineTo(points[a], points[a + 1]);
      }
  ...
  private void drawOuterFrameRemainPart(
      Canvas canvas,
      float centerX,
      float centerY,
      float frameWidth,
    ...
    for (int a = 0; a < points.length - 1; a += 2) {
      if (a == 0 || a == 4 || (excludeInnerLines && a == 2)) {
        path.moveTo(points[a], points[a + 1]);
      } else {
        path.lineTo(points[a], points[a + 1]);
      }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    float closingPercentage =
        closingPercentageWhileMoving == null
            ? this.closingPercentage
            : closingPercentageWhileMoving.floatValue();

    float rotationXoffset = closingPercentageToXoffset(closingPercentage);

    drawOuterFrameRemainPart(
        canvas,
        centerX,
        centerY,
        windowWidth,
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_UP:
        if (onClosingPercentageChangeListener != null) {
          onClosingPercentageChangeListener.onClosingPercentageChanged(
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (Math.abs(x - lastTouchX) < Math.abs(delta)) {
          ...
          if (closingPercentageWhileMoving < 0) closingPercentageWhileMoving = 0f;
          else if (closingPercentageWhileMoving > 100) closingPercentageWhileMoving = 100f;
  ...
  public void setOnClosingPercentageChangeListener(
      OnClosingPercentageChangeListener onClosingPercentageChangeListener) {
  ...
  public interface OnClosingPercentageChangeListener {
    ...
    void onClosingPercentageChangeing(
