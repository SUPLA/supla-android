...
@Composable
fun ThermostatGeneralScreen(
  item: ItemBundle,
  viewModel: ThermostatGeneralViewModel = hiltViewModel()
) {
  ViewModelHost(
    viewModel = viewModel,
    onCreate = { viewModel.observeData(item.remoteId, item.deviceId) },
    onResume = { viewModel.loadData(item.remoteId, item.deviceId) }
  ) {
