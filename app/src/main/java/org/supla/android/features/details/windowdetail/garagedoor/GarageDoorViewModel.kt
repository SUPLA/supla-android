...
@HiltViewModel
class GarageDoorViewModel @Inject constructor(
  ...
  override fun handleGroup(group: GroupData) {
    updateState { state ->
      ...
      val positions = group.groupDataEntity.channelGroupEntity.getGarageDoorPositions()
...
private fun ChannelGroupEntity.getGarageDoorPositions(): List<Float> =
  groupTotalValues.mapNotNull {
    val (value) = guardLet(it as? ShadingSystemGroupValue) { return@mapNotNull null }
    value.position.toFloat()
  }

data class GarageDoorViewModelState(
  override val remoteId: Int? = null,
  override val windowState: GarageDoorState = GarageDoorState(WindowGroupedValue.Similar(0f)),
