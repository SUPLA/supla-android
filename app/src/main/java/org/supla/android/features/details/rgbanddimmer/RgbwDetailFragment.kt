...
@AndroidEntryPoint
class RgbwDetailFragment :
  BaseDetailFragment<RgbwDetailViewState, RgbwDetailViewEvent>(R.layout.fragment_standard_detail),
  ToolbarItemsClickHandler {

  override val viewModel: RgbwDetailViewModel by viewModels()
  ...
  override fun isCloseEvent(event: RgbwDetailViewEvent) = event == RgbwDetailViewEvent.Close

  override fun updateToolbarTitle(state: RgbwDetailViewState) {
    state.caption?.let { setToolbarTitle(it(requireContext())) }
  }

  override fun onStart() {
    super.onStart()
    setToolbarItemVisible(R.id.toolbar_settings, viewState.hasSettings)
  }

  override fun onStop() {
    super.onStop()
    setToolbarItemVisible(R.id.toolbar_settings, false)
  }

  override fun handleViewState(state: RgbwDetailViewState) {
    super.handleViewState(state)
    setToolbarItemVisible(R.id.toolbar_settings, state.hasSettings)
  }

  override fun onMenuItemClick(menuItem: MenuItem): Boolean {
    if (menuItem.itemId == R.id.toolbar_settings) {
      navigator.navigateTo(R.id.legacy_dimmer_settings_fragment, bundle(item))
      return true
    }
