...
@Immutable
data class DataSelectionDialogState(
...
@Composable
fun DataSelectionDialog(
  state: DataSelectionDialogState,
...
@Composable
private fun Content(localState: MutableState<DataSelectionDialogState>) {
  localState.value.spinner?.let { list ->
    TextSpinner(
      options = list,
      onOptionSelected = {
        val newCheckbox =
  ...
  localState.value.checkbox?.let { list ->
    list.label?.let {
      Text(
...
@Composable
private fun Checkbox(checked: Boolean, color: Color, label: String, onCheckedChange: (Boolean) -> Unit) {
  ...
  SuplaTheme {
    DataSelectionDialog(
      state = DataSelectionDialogState(
        ...
        checkbox = MultipleSelectionList(setOf(PhaseItem.PHASE_1, PhaseItem.PHASE_2), PhaseItem.ALL, R.string.details_em_phases)
      ),
    )
  }
}
