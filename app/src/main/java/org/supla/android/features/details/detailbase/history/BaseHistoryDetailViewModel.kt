...
abstract class BaseHistoryDetailViewModel(
    ...
    updateState { state ->
      state.chartData.sets.firstOrNull { it.remoteId == remoteId }?.let { channelSets ->
        if (channelSets.function.hasCustomFilters()) {
          state.copy(dataSelectionDialogState = provideSelectionDialogState(channelSets, state.chartCustomFilters))
  ...
  ): DataSelectionDialogState {
    return DataSelectionDialogState(channelSets.name)
  }

  override fun hideSelection() {
    updateState { it.copy(dataSelectionDialogState = null) }
  }
  ...
  protected open fun loadChartState(profileId: Long, remoteId: Int): ChartState =
    userStateHolder.getChartState(profileId, remoteId) { gson, string -> gson.fromJson(string, DefaultChartState::class.java) }
      ?: DefaultChartState.default()
  ...
  protected fun restoreRange(chartState: ChartState) {
    ...
    updateState { state ->
      state.copy(
        filters = setupFilters(state.filters, selectedRange, dateRange, chartState),
        range = dateRange,
...
data class HistoryDetailViewState(
  ...
  val dataSelectionDialogState: DataSelectionDialogState? = null,
