...
class HumidityDetailViewModel @Inject constructor(
...
) : StandardDetailViewModel<HumidityDetailViewState, HumidityDetailViewEvent>(
  ...
  override fun closeEvent() = HumidityDetailViewEvent.Close

  override fun updatedState(state: HumidityDetailViewState, channelDataBase: ChannelDataBase) =
    state.copy(caption = getCaptionUseCase(channelDataBase.shareable))
}

sealed interface HumidityDetailViewEvent : StandardDetailViewEvent {
  data object Close : HumidityDetailViewEvent
}

data class HumidityDetailViewState(
