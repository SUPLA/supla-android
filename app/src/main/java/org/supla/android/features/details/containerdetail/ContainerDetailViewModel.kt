package org.supla.android.features.details.containerdetail
/*
...
) : StandardDetailViewModel<ContainerDetailViewState, ContainerDetailViewEvent>(
  readChannelByRemoteIdUseCase,
  readChannelGroupByRemoteIdUseCase,
  updateEventsManager,
  preferences,
  ContainerDetailViewState(),
  schedulers
) {

  override fun closeEvent() = ContainerDetailViewEvent.Close

  override fun updatedState(state: ContainerDetailViewState, channelDataBase: ChannelDataBase) =
    state.copy(caption = getCaptionUseCase(channelDataBase.shareable))
}

sealed interface ContainerDetailViewEvent : StandardDetailViewEvent {
  data object Close : ContainerDetailViewEvent
}

data class ContainerDetailViewState(
