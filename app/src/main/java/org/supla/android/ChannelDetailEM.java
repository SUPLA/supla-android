...
@AndroidEntryPoint
public class ChannelDetailEM extends DetailLayout
  ...
  protected void init() {
    ...
    tvPhaseForwardActiveEnergyBalanced = findViewById(R.id.emtv_PhaseForwardActiveEnergyBalanced);
    tvlPhaseForwardActiveEnergyBalanced = findViewById(R.id.emtv_lPhaseForwardActiveEnergyBalanced);
    tvPhaseReverseActiveEnergyBalanced = findViewById(R.id.emtv_PhaseReverseActiveEnergyBalanced);
    tvlPhaseReverseActiveEnergyBalanced = findViewById(R.id.emtv_lPhaseReverseActiveEnergyBalanced);
  ...
  private void displayMeasurementDetails(long vars, boolean sum) {
    displayMeasurementDetail(vars, SuplaConst.EM_VAR_FREQ, tvFreq, tvlFreq);

    displayMeasurementDetail(vars, sum ? 0 : SuplaConst.EM_VAR_VOLTAGE, tvVoltage, tvlVoltage);

    displayMeasurementDetail(
        vars,
        sum ? 0 : SuplaConst.EM_VAR_CURRENT,
    ...
    displayMeasurementDetail(
        vars,
        SuplaConst.EM_VAR_POWER_ACTIVE,
        SuplaConst.EM_VAR_POWER_ACTIVE_KW,
        tvPowerActive,
        tvlPowerActive);
    displayMeasurementDetail(
        vars,
        SuplaConst.EM_VAR_POWER_REACTIVE,
        SuplaConst.EM_VAR_POWER_REACTIVE_KVAR,
        tvPowerReactive,
        tvlPowerReactive);
    displayMeasurementDetail(
        vars,
        SuplaConst.EM_VAR_POWER_APPARENT,
        SuplaConst.EM_VAR_POWER_APPARENT_KVA,
        tvPowerApparent,
        tvlPowerApparent);

    displayMeasurementDetail(
        vars, sum ? 0 : SuplaConst.EM_VAR_POWER_FACTOR, tvPowerFactor, tvlPowerFactor);
    displayMeasurementDetail(
        vars, sum ? 0 : SuplaConst.EM_VAR_PHASE_ANGLE, tvPhaseAngle, tvlPhaseAngle);

    displayMeasurementDetail(
        vars,
        SuplaConst.EM_VAR_FORWARD_ACTIVE_ENERGY,
        tvPhaseForwardActiveEnergy,
        tvlPhaseForwardActiveEnergy);
    displayMeasurementDetail(
        vars,
        SuplaConst.EM_VAR_REVERSE_ACTIVE_ENERGY,
        tvPhaseReverseActiveEnergy,
        tvlPhaseReverseActiveEnergy);
    displayMeasurementDetail(
        vars,
        SuplaConst.EM_VAR_FORWARD_REACTIVE_ENERGY,
        tvPhaseForwardReactiveEnergy,
        tvlPhaseForwardReactiveEnergy);
    displayMeasurementDetail(
        vars,
        SuplaConst.EM_VAR_REVERSE_REACTIVE_ENERGY,
        tvPhaseReverseReactiveEnergy,
        tvlPhaseReverseReactiveEnergy);

    mBalanceAvailable =
        ((vars & SuplaConst.EM_VAR_FORWARD_ACTIVE_ENERGY_BALANCED) > 0
    ...
    if (sum && mBalanceAvailable) {
      ...
      displayMeasurementDetail(
          vars,
          SuplaConst.EM_VAR_FORWARD_ACTIVE_ENERGY_BALANCED,
          tvPhaseForwardActiveEnergyBalanced,
          tvlPhaseForwardActiveEnergyBalanced);
      displayMeasurementDetail(
          vars,
          SuplaConst.EM_VAR_REVERSE_ACTIVE_ENERGY_BALANCED,
  ...
  public void channelDataToViews(Channel channel) {
    ...
    if (cev != null
      ...
      if (mDBH.electricityMeterMeasurementsStartsWithTheCurrentMonth(channel.getChannelId())) {
        ...
        double v0 = mDBH.getLastElectricityMeterMeasurementValue(0, channel.getChannelId(), false);
        double v1 = mDBH.getLastElectricityMeterMeasurementValue(-1, channel.getChannelId(), false);
        currentConsumption = v0 - v1;
        currentCost = currentConsumption * em.getPricePerUnit();

        v0 = mDBH.getLastElectricityMeterMeasurementValue(0, channel.getChannelId(), true);
        v1 = mDBH.getLastElectricityMeterMeasurementValue(-1, channel.getChannelId(), true);
        currentProduction = v0 - v1;
      }

      if (chartHelper.isProductionDataSource()) {
        tvTotalActiveEnergy.setText(
            getActiveEnergyFormattedString(sum.getTotalReverseActiveEnergy()));
        tvCurrentConsumptionProduction.setText(String.format("%.2f kWh", currentProduction));
      } else {
        tvTotalActiveEnergy.setText(
            getActiveEnergyFormattedString(sum.getTotalForwardActiveEnergy()));
      ...
      tvCurrentCost.setText(String.format("%.2f " + em.getCurrency(), currentCost));
      mArrowsVisible = (em.getMeasuredValues() & SuplaConst.EM_VAR_REVERSE_ACTIVE_ENERGY) > 0;
  ...
  private void runDownloadTask() {
    ...
    if (demm == null) {
      demm =
          new DownloadElectricityMeterMeasurements(
