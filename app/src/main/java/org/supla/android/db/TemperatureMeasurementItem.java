...
public class TemperatureMeasurementItem extends MeasurementItem {
  ...
  public void AssignCursorData(Cursor cursor) {
    ...
    setChannelId(
        cursor.getInt(
            cursor.getColumnIndex(SuplaContract.TemperatureLogEntry.COLUMN_NAME_CHANNELID)));

    setTimestamp(
        cursor.getLong(
            cursor.getColumnIndex(SuplaContract.TemperatureLogEntry.COLUMN_NAME_TIMESTAMP)));

    setTemperature(
        cursor.getDouble(
            cursor.getColumnIndex(SuplaContract.TemperatureLogEntry.COLUMN_NAME_TEMPERATURE)));

    setProfileId(
        cursor.getLong(
            cursor.getColumnIndex(SuplaContract.TemperatureLogEntry.COLUMN_NAME_PROFILEID)));
  }
  ...
  public ContentValues getContentValues() {
    ...
    putNullOrDouble(
        values, SuplaContract.TemperatureLogEntry.COLUMN_NAME_TEMPERATURE, getTemperature());
