...
public class TempHumidityMeasurementItem extends MeasurementItem {
  ...
  public void AssignCursorData(Cursor cursor) {
    ...
    setChannelId(
        cursor.getInt(
            cursor.getColumnIndex(SuplaContract.TempHumidityLogEntry.COLUMN_NAME_CHANNELID)));

    setTimestamp(
        cursor.getLong(
            cursor.getColumnIndex(SuplaContract.TempHumidityLogEntry.COLUMN_NAME_TIMESTAMP)));

    setTemperature(
        cursor.getDouble(
            cursor.getColumnIndex(SuplaContract.TempHumidityLogEntry.COLUMN_NAME_TEMPERATURE)));

    setTemperature(
        cursor.getDouble(
            cursor.getColumnIndex(SuplaContract.TempHumidityLogEntry.COLUMN_NAME_HUMIDITY)));
    setProfileId(
        cursor.getLong(
            cursor.getColumnIndex(SuplaContract.TempHumidityLogEntry.COLUMN_NAME_PROFILEID)));
  }
  ...
  public ContentValues getContentValues() {
    ...
    putNullOrDouble(
        values, SuplaContract.TempHumidityLogEntry.COLUMN_NAME_TEMPERATURE, getTemperature());

    putNullOrDouble(values, SuplaContract.TempHumidityLogEntry.COLUMN_NAME_HUMIDITY, getHumidity());
