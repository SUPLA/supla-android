...
public class DiwCalibrationTool extends DimmerCalibrationTool
    ...
    public final static int DIW_CMD_CONFIG_COMPLETE = 0x3;
    ...
    public final static int DIW_CMD_SET_INPUT_BEHAVIOR = 0x7;
    public final static int DIW_CMD_SET_INPUT_MODE = 0x8;
    public final static int DIW_CMD_SET_INPUT_BI_MODE = 0x9;
    ...
    protected void setInputOptionBtnApparance(boolean selected,
                                      int resIdNormal, int resIdSelected) {

        Drawable bg =  getResources().getDrawable(selected ? R.drawable.rounded_option_sel_btn
    ...
    private void setInputOptionSelected(int mode, boolean selected) {
        ...
        if (mode == DiwCfgParameters.INPUT_MODE_MONOSTABLE) {
            setInputOptionBtnApparance(selected,
                    R.drawable.infinity, R.drawable.infinitywhite);
        } else {
            setInputOptionBtnApparance(selected,
                    R.drawable.p100, R.drawable.p100white);
        }
    }
    ...
    private void setInputMode() {
        setInputMode(cfgParameters.getInputMode(),
                cfgParameters.getInputBehavior(),
                cfgParameters.getInputBiMode());
    }
    ...
    private byte optionToByteValue() {
        if (imgInputOption.getTag() != null) {
            ...
            if (tag.getInputMode() == DiwCfgParameters.INPUT_MODE_MONOSTABLE) {
                return tag.isSelected() ? (byte)DiwCfgParameters.BEHAVIOR_LOOP
                        : (byte)DiwCfgParameters.BEHAVIOR_NORMAL;
            } else {
                return tag.isSelected() ? (byte)DiwCfgParameters.BISTABLE_MODE_100P
    ...
    public void onClick(View v) {
        ...
        if (v == imgInputOption && imgInputOption.getTag() != null) {
            ...
            if (tag.getInputMode() == DiwCfgParameters.INPUT_MODE_BISTABLE) {
                calCfgRequest(DIW_CMD_SET_INPUT_BI_MODE, (byte) (optionToByteValue() & 0xFF),
                        null);
            } else {
                calCfgRequest(DIW_CMD_SET_INPUT_BEHAVIOR, (byte) (optionToByteValue() & 0xFF),
                        null);
            }
        }
    }
