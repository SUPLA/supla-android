...
public class ZWaveConfigurationWizardActivity extends WizardActivity
  ...
  private void configModeNotificationTimerActivate() {
    ...
    mConfigModeNoficationTimer.scheduleAtFixedRate(
        new TimerTask() {
          @Override
          public void run() {
            runOnUiThread(
  ...
  private void wathdogActivate(final int timeoutSec, final int msgResId, boolean calCfg) {
    ...
    if (calCfg) {
      ...
      mAnyCalCfgResultWatchdogTimer.scheduleAtFixedRate(
          new TimerTask() {
            @Override
            public void run() {
    ...
    mWatchdogTimer.scheduleAtFixedRate(
        new TimerTask() {
          @Override
          public void run() {
            runOnUiThread(
  ...
  private void showWaitMessage(
    ...
    mWaitMessagePreloaderTimer.scheduleAtFixedRate(
        new TimerTask() {
          @Override
          public void run() {
            runOnUiThread(
  ...
  protected void onChannelBasicCfg(SuplaChannelBasicCfg basicCfg) {
    ...
    if (mChannelBasicCfgToFetch.size() > 0) {
      ...
      } else if (getVisiblePageId() == PAGE_SELECT_CHANNEL && mDeviceList.size() == 0) {
