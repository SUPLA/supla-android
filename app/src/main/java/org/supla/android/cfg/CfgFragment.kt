...
class CfgFragment : Fragment() {
  ...
  override fun onCreateView(
    ...
    arrayOf(
      ...
      binding.locationOrderingButton
    ).forEach {
      it.setTypeface(type)
    }
    ...
    binding.temperatureUnit.setOnPositionChangedListener() { pos ->
      when (pos) {
    ...
    binding.channelHeight.setOnPositionChangedListener() { pos ->
      when (pos) {
    ...
    binding.showOpeningMode.position =
      if (viewModel.cfgData.showOpeningPercent.value == true) 0 else 1
    binding.showOpeningMode.setOnPositionChangedListener { pos ->
      when (pos) {
