...
public class SuplaThermostatScheduleCfg {
    ...
    private void setHourValue(@HourValueType int type,
                              @DayOfWeek int weekday, short hour, byte value) {
        if (hour < 0) {
            hour = 0;
        } else if (hour > 23) {
            hour = 23;
        ...
        for (a = 0; a < mGroups.size(); a++) {
            group = mGroups.get(a);
            if (group.mValueType == type
                    && Arrays.equals(group.mHourValue, hourValue)) {
                group.mWeekDays |= weekday;
                break;
            }
        }
    ...
    public @DayOfWeek
    int getWeekdayByDayIndex(short idx) {
        switch (idx) {
            case 2:
    ...
    @IntDef({DayOfWeek.SUNDAY, DayOfWeek.MONDAY, DayOfWeek.TUESDAY, DayOfWeek.WEDNESDAY,
