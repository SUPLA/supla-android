...
public class SuplaCfg {
  ...
  void setName(String Name) {
    this.Name =
        Name == null
            ? ""
            : Name.substring(0, Math.min(Name.length(), SuplaConst.SUPLA_CLIENT_NAME_MAXSIZE - 1));
  }
  ...
  void setAccessIDpwd(String AccessIDpwd) {
    this.AccessIDpwd =
        AccessIDpwd == null
            ? ""
            : AccessIDpwd.substring(
                0, Math.min(AccessIDpwd.length(), SuplaConst.SUPLA_ACCESSID_PWD_MAXSIZE - 1));
  }
  ...
  void setEmail(String Email) {
    this.Email =
        Email == null
            ? ""
            : Email.substring(0, Math.min(Email.length(), SuplaConst.SUPLA_EMAIL_MAXSIZE - 1));
  }
  ...
  void setPassword(String Password) {
    this.Password =
        Password == null
            ? ""
            : Password.substring(
                0, Math.min(Password.length(), SuplaConst.SUPLA_PASSWORD_MAXSIZE - 1));
  }
  ...
  void setSoftVer(String SoftVer) {
    this.SoftVer =
        SoftVer == null
            ? ""
            : SoftVer.substring(
                0, Math.min(SoftVer.length(), SuplaConst.SUPLA_SOFTVER_MAXSIZE - 1));
  }
}
