...
public class SuplaChannelState implements Serializable {
  ...
  public SuplaChannelState(
      ...
      byte batteryPowered,
      byte wiFiRSSI,
      byte wiFiSignalStrength,
      byte bridgeNodeOnline,
      byte bridgeNodeSignalStrength,
      int uptime,
      int connectionUptime,
      byte batteryHealth,
      byte lastConnectionResetCause,
      int lightSourceLifespan,
    ...
    if ((fields & FIELD_BATTERYLEVEL) > 0 && batteryLevel >= 0 && batteryLevel <= 100) {
    ...
    if ((fields & FIELD_BATTERYHEALTH) > 0 && batteryHealth >= 0 && batteryHealth <= 100) {
  ...
  public String getIpv4String() {
    ...
    return String.format(
        "%d.%d.%d.%d", (ipv4 & 0xff), (ipv4 >> 8 & 0xff), (ipv4 >> 16 & 0xff), (ipv4 >> 24 & 0xff));
  }
  ...
  public String getLightSourceLifespanString() {
    ...
    if (percentLeft != null) {
      return String.format("%dh (%.2f%%)", getLightSourceLifespan(), percentLeft);
    }
  ...
  public String getLightSourceOperatingTimeString() {
    ...
    return String.format(
        "%02dh %02d:%02d", timeSec / 3600, timeSec % 3600 / 60, timeSec % 3600 % 60);
  }
}
