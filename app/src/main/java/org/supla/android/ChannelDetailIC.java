...
public class ChannelDetailIC extends DetailLayout
    implements SuplaRestApiClientTask.IAsyncResults,
  ...
  protected void init() {
    ...
    ArrayAdapter<String> adapter =
        new ArrayAdapter<>(
            ...
            chartHelper.getMasterSpinnerItems(13));
  ...
  private void updateSlaveSpinnerItems() {
    ...
    List<String> itemsList = chartHelper.getSlaveSpinnerItems(icSpinnerMaster);
    String[] items = itemsList.toArray(new String[0]);
    Object previousSelection = icSpinnerSlave.getSelectedItem();
  ...
  private void channelExtendedDataToViews() {
    ...
    if (cev != null
      ...
      if (mDBH.impulseCounterMeasurementsStartsWithTheCurrentMonth(channel.getChannelId())) {
        ...
        double v0 = mDBH.getLastImpulseCounterMeasurementValue(0, channel.getChannelId());
        double v1 = mDBH.getLastImpulseCounterMeasurementValue(-1, channel.getChannelId());
  ...
  private void runDownloadTask() {
    ...
    if (dtm == null) {
      dtm =
          new DownloadImpulseCounterMeasurements(
