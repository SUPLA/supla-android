...
public class SuplaRollerShutter extends View {
  ...
  private void init() {
    ...
    FrameLineWidth = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (float) 2, metrics);

    Spaceing = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (float) 3, metrics);
    ...
    */
    ...
    if (!Moving && percent == 0 && Markers != null && !Markers.isEmpty()) {
      ...
      for (int a = 0; a < Markers.size(); a++) {
        pos =
            (float) ((getHeight() - markerHalfHeight * 2) * Markers.get(a) / 100.00)
                + markerHalfHeight;
    ...
    if (percent == 0 || LouverCount <= 0) return;
    ...
    paint.setStyle(Paint.Style.STROKE);
    paint.setColor(
        Moving ? (WindowFrameLineColor & 0x00ffffff) | (50 << 24) : WindowFrameLineColor);
    paint.setStrokeWidth(FrameLineWidth);

    hFLW = FrameLineWidth / 2;

    float LouverHeight =
        (getHeight() - LouverSpaceing * (LouverCount - 1)) / LouverCount - FrameLineWidth;
    h -= hFLW;
    ...
    for (int a = 0; a < _LouverCount; a++) {
      ...
      if (bottomPosition > 0 && percent > bottomPosition) {
        float n =
            (_LouverCount - 1) - ((100 - percent) / ((100 - bottomPosition) / (_LouverCount - 1)));
    ...
    h = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, (float) 0.5, metrics);
    ...
    switch (action) {
      ...
      case MotionEvent.ACTION_MOVE:
        ...
        if (Math.abs(x - MoveX) < Math.abs(delta)) {
          ...
          if (delta > 0) virtPercent += p;
          else virtPercent -= p;

          if (virtPercent < 0) virtPercent = 0;
          else if (virtPercent > 100) virtPercent = 100;
