...
public class SuplaRangeCalibrationWheel extends View {
  ...
  private void drawBtnLines(Canvas canvas, RectF rectF) {
    ...
    for (int a = 0; a < lc; a++) {
      ...
      lineRectF.set(
          rectF.left,
          rectF.top + step * a - width / 2,
          rectF.right,
          rectF.top + step * a + width / 2);

      canvas.drawRoundRect(lineRectF, 15, 15, paint);
    }
  }
  ...
  private PointF drawButton(Canvas canvas, double rad, boolean visible) {
    ...
    rectF.set(
        x - halfBtnSize, wheelCenterY - halfBtnSize, x + halfBtnSize, wheelCenterY + halfBtnSize);
  ...
  private void drawValue(Canvas canvas) {
    ...
    if (boostVisible) {
      ...
      float hh = ((bottom - top) + borderLineWidth * 2.0f) * (float) boostLineHeightFactor / 2.0f;

      paint.setColor(boostLineColor);
      canvas.drawLine(vleft, btnLeftCenter.y - hh, vleft, btnLeftCenter.y + hh, paint);
    }
  }
  ...
  private boolean btnTouched(PointF btnCenter, PointF touchPoint) {
    if (btnCenter != null) {
      float touchRadius =
          (float)
              Math.sqrt(
                  Math.pow(touchPoint.x - btnCenter.x, 2)
  ...
  private double touchPointToRadian(PointF touchPoint) {
    return Math.atan2(touchPoint.y - wheelCenterY, touchPoint.x - wheelCenterX);
  }
