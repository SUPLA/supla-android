...
public class DigiglassController extends View {
    ...
    private void init() {
      ...
      if (r!=null) {
          pxPerDP = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                  1f, r.getDisplayMetrics());
      }
    ...
    public DigiglassController(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void drawDots(Canvas canvas, RectF rect) {
        ...
        for(int a=0;a<ch;a++) {
            for(int b=0;b<cw;b++) {
                if (b%2 != a%2) {
                    canvas.drawCircle(rect.left+wmargin+fieldRadius+b*diameter,
                            rect.top+hmargin+fieldRadius+a*diameter, pointRadius, paint);
                }
            }
        }
    }
    ...
    private void drawButton(Canvas canvas, RectF rect, boolean lines) {
        ...
        if (lines) {
            ...
            canvas.drawRect(centerx - p/2f, centery-r/2, centerx+p/2f,
                    centery+r/2, paint);

            p *= 0.6f;
            canvas.drawRect(centerx - p/2f, centery-distance-r/2, centerx+p/2f,
                    centery-distance+r/2, paint);
        ...
        for(a=1;a<5;a++) {
            ...
            canvas.drawCircle(cx,
                    centery-(distance/2f+distance), m,
                    paint);
        ...
        for(a=2;a<4;a++) {
            ...
            canvas.drawCircle(cx,
                    centery-(distance/2f+distance*2), m,
                    paint);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawLine(0,lineHalfWidth,
                width,lineHalfWidth, paint);
        canvas.drawLine(0,height - lineHalfWidth,
                width,height - lineHalfWidth, paint);

        canvas.drawLine(0,barHeight - lineHalfWidth,
                width,barHeight - lineHalfWidth, paint);
        canvas.drawLine(0,height - barHeight + lineHalfWidth,
                width,height - barHeight + lineHalfWidth, paint);

        canvas.drawLine(lineHalfWidth,barHeight - lineHalfWidth,
                lineHalfWidth,height - barHeight + lineHalfWidth, paint);
        canvas.drawLine(width-lineHalfWidth,barHeight - lineHalfWidth,
                width-lineHalfWidth,height - barHeight + lineHalfWidth, paint);
    }
            ...
            for(int a=0;a<sectionCount;a++) {
                ...
                if (rectF.contains(event.getX(), event.getY())) {
                    ...
                    if (onSectionClickListener != null) {
                        onSectionClickListener.onGlassSectionClick(this, a,
                                isSectionTransparent(a));
                    }
                    break;
                }
            }
