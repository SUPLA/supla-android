...
class GetAllChannelIssuesUseCase(
  ...
  operator fun invoke(channelWithChildren: ChannelWithChildren): List<ChannelIssueItem> =
    mutableListOf<ChannelIssueItem>().apply {
      getChannelLowBatteryIssueUseCase(channelWithChildren)?.let { add(it) }
      addAll(getChannelSpecificIssuesUseCase(channelWithChildren))
    }
}
